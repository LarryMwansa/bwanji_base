...
export default function SocialFormatter() {
  ...
  function applyUnicodeStyle(style) {
    ...
    if (style === "strikethrough") {
      ...
      styled = selected
        .split("")
        .map((ch) => unicodeMaps[style][ch] || ch)
        .join("");
    }
  ...
  function insertBullet() {
    ...
    const newText =
      before.slice(0, insertPos) + "• " + before.slice(insertPos) + after;
    setInput(newText);
    setTimeout(() => {
      textarea.focus();
  ...
  return (
    <main className={styles.container}>
      ...
      <div className={styles.topbar}>
        <button type="button" onClick={() => applyUnicodeStyle("bold")}>
          Bold
        </button>
        <button type="button" onClick={() => applyUnicodeStyle("italic")}>
          Italic
        </button>
        <button
          type="button"
          onClick={() => applyUnicodeStyle("strikethrough")}
        >
          Strikethrough
        </button>
        <button type="button" onClick={() => applyUnicodeStyle("monospace")}>
          Monospace
        </button>
              ...
              .map((chunk, idx) => {
                if (/^\n+$/.test(chunk)) {
                  ...
                  return Array(chunk.length)
                    .fill()
                    .map((_, i) => (
                      <div key={idx + "-" + i} style={{ height: "1em" }}></div>
                    ));
                }
